...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/14 16:32:12 by lilam             #+#    #+#             */
/*   Updated: 2018/04/14 16:59:31 by lilam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"
#define VAR char *split; char **line_split; int x = 0; int i = 0
#define VAR2 char *line; int ret; int x
...
int	count_x(char *line)
{
	...
	while (line[i])
	{
		if ((ft_isdigit(line[i]) || ft_isalpha(line[i])) && (line[i + 1] == ' '
					|| line[i + 1] == '\0'))
			count++;
		i++;
	}
...
int	**parse_line(char *line, char *hex_color, int **coords)
{
	...
	while (line_split[i])
	{
		...
		else if ((coords[i][0] = ft_atoi(line_split[i])))
		{
			while (*split != 'x')
				split++;
